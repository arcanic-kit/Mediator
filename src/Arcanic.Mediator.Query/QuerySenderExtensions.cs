...
public static class QuerySenderExtensions
{
    public static async Task<TQueryResponse> SendAsync<TQueryResponse>(this IQuerySender querySender, IQuery<TQueryResponse> query, CancellationToken cancellationToken = default)
    {
        ...
        var dispatcher = querySender.RequestDispatchers.GetOrAdd(queryType, static requestType =>
        {
        ...
        var result = await dispatcher.DispatchAsync(query, querySender.ServiceProvider, cancellationToken);
