...
public sealed class MessageMediatorRequestPipelineHandlerStrategy<TMessage, TMessageResult> : IMessageMediatorStrategy<TMessage, Task<TMessageResult>> where TMessage : notnull
{
    ...
    {
        // Get registered pipeline behaviors for this message type
        ...
        RequestPipelineDelegate<TMessageResult> handlerExecution = async () =>
        {
            return await ExecuteHandlerPipelineAsync(message, handlerProvider, context);
        };
        ...
        var task = handlerExecution() as Task;
        return await (Task<TMessageResult>)(object)task!;
    }
